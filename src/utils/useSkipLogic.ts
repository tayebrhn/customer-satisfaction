...
export function useSkipLogic({
  ...
  const evaluateRule = useCallback(
    (rule: LogicRule, formValues: Record<string, any>) => {
      ...
      const comparisonValue = rule.trigger_value;
      ...
      if (rule.operator === "IN") {
        return Array.isArray(comparisonValue) && comparisonValue.includes(triggerAnswer);
      }
      ...
      if (rule.operator === "NOT_EQUALS") return triggerAnswer != comparisonValue;
  ...
  const recalculateVisibility = useCallback((valuesOverride?: Record<string, any>) => {
    ...
    if (hasChanged) {
      ...
      prevVisibleSet.forEach((questionNum) => {
        if (!newVisibleSet.has(questionNum)) {
          ...
          setValue(fieldName, null, { shouldValidate: false, shouldDirty: false });
          
          const otherField = `${fieldName}_other`;
          if (currentAnswers[otherField] !== undefined) {
             setValue(otherField, "", { shouldValidate: false, shouldDirty: false });
          }
        }
      });
    }
  ...
  }, [allQuestions, isQuestionVisible, getValues, setValue, onQuestionVisibilityChange]);
  ...
  const handleAnswerChangeWithLogic = useCallback(
    async (questionNumber: number, value: any, shouldValidate = false) => {
      ...
      // 4. Handle Validation
      if (shouldValidate) {
        const question = allQuestions.find((q) => q.sequence_num === questionNumber);
  ...
  const visibleCategoryQuestions = useMemo(() => {
    ...
    return currentCategory.questions.filter((q) => visibleQuestions.has(q.sequence_num));
  ...
  return {
    ...
    isQuestionVisible: (q: SurveyQuestion) => visibleQuestions.has(q.sequence_num),
